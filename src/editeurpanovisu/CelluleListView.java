...
public class CelluleListView extends ListCell<String> {

    private final ImageView imageView = new ImageView();
    private int iCell = 0;

    public CelluleListView() {
        ...
        setOnDragDropped(evenement -> {
            ...
            if (db.hasString()) {
                ...
                if (draggedIdx < thisIdx) {
                    i1 = draggedIdx;
                    i2 = thisIdx;
                    CellulePanoramique cpDragged = cellulesPanoramiques.get(draggedIdx);
                    String strItems = items.get(draggedIdx);
                    for (int i = i1; i < i2; i++) {
                        String strTemp = items.get(i + 1);
                        items.set(i, strTemp);
                        CellulePanoramique temp = cellulesPanoramiques.get(i + 1);
                    ...
                    for (int i = i2 - 1; i >= i1; i--) {
                        ...
                        CellulePanoramique temp = cellulesPanoramiques.get(i);
